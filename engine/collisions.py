...
def collision(app, type, surrounding, vel, aabb, tileaabb, dt):
    ...
    for k, v in surrounding.items():
        if v is not None and type=="solid" and v=="solid":
            check[k] = False
    ...
    if type=="solid":
        ...
        if (aabb.t <= tileaabb.b and oaabb.t > tileaabb.b - resolve2/2) and check["b"] and vel.y<=0:
            response["move"] = aabb.setT(tileaabb.b + resolve)
            response["collided"] = True
            response["top"] = True
    elif (type=="onlyB" or type=="platform"):
        if (aabb.b >= tileaabb.t and oaabb.b < tileaabb.t + resolve2/2) and check["t"] and vel.y>=0:
            response["move"] = aabb.setB(tileaabb.t - resolve)
            response["collided"] = True
            response["bottom"] = True
    elif (type=="onlyL"):
        if (aabb.l <= tileaabb.r and oaabb.l > tileaabb.r - resolve2/2) and check["r"] and vel.x<=0:
            response["move"] = aabb.setL(tileaabb.r + resolve)
            response["collided"] = True
            response["left"] = True
    elif (type=="onlyR"):
        if (aabb.r >= tileaabb.l and oaabb.r < tileaabb.l + resolve2/2) and check["l"] and vel.x>=0:
            response["move"] = aabb.setR(tileaabb.l - resolve)
            response["collided"] = True
            response["right"] = True
